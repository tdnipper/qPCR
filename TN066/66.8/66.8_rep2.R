...
df <- read.csv("TN066/66.8/TN066.8_rep2_filtered.csv")
...
p <- ggplot(data_enrich_long, aes(x = Target.Name, y = normalized_foldchange, color = Condition)) +
	...
	theme_classic() + 
	ylim(0, 5)
ggsave("TN066/66.8/66.8_rep2_foldchange.png", plot = p, width = 10, height = 6, dpi=300)


# df_amp <- read.csv("TN066/66.8/TN066.8_rep2_amp.csv") %>%
# 	select(-Well) %>%
# 	drop_na() %>%
