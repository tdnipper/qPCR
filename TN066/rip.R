...
data_dct <- data %>%
  ...
  mutate(across(.cols = starts_with("delta_ct_"),
                .fns = ~ 2^-.x,
                .names = "enrichment_{.col}")) %>%
  rename_with(~ sub("delta_ct_", "", .x), starts_with("enrichment_")) %>%
  select(`Target Name`, starts_with("enrichment_")) %>%
...
p <- ggplot(data_dct, aes(x = `Target Name`, y = enrichment, color = Condition)) +
  geom_point(size = 2, alpha = 0.7) +
  labs(title = "NP Enrichment Infected vs Mock",
    x = "Target Gene",
    y = "% Input",
    color = "Status") +
  theme_classic() +
...
ggsave("TN066/percent_input.png", plot = p, width = 10, height = 6, dpi = 300)
