...
output.to_excel("TN031.9 redo ddCT.xlsx")
...
melt_curve = pd.read_excel(
    "TN031_9_redo.xlsx", sheet_name="Melt Curve Raw Data", skiprows=46
)
...
def plot_melt_curve(df: pd.DataFrame, title: str) -> plt:
    ...
    plt.tight_layout()
    plt.savefig(f"TN031.9 Combined (Redo) - {title}.png", dpi=300)
